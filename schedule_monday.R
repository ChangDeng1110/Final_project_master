################# Monday ###################
set.seed(100)
temp_75_mid <- read.csv("temprature_all.csv")[c(6361:6409),] #9.2
...
ggplot(print_table_opt1, aes(time_data),na.rm=T,group=1)+
  geom_rect(aes(xmin=as.POSIXct("2017-09-02 09:00:00 AEST"),
                xmax=as.POSIXct("2017-09-02 18:00:00 AEST"), ymin=20, ymax=21, fill="Occupy"), alpha=1) +
  geom_rect(aes(xmin=as.POSIXct("2017-09-02 6:00:00 AEST"),
                xmax=as.POSIXct("2017-09-02 9:00:00 AEST"), ymin=20, ymax=21, fill="No-occupy"), alpha=1) +
  ...
  labs(title = "Sep 2", x = "Time", y = "Temperature (Celsius)",legend.text = element_text(size = 18))+
...
check_1 <- print_table_opt1[1:720,]
line_fore <- check_1[,2]
line_time <- check_1[,1]
forecast <- append(line_fore,check_1[,3]) 
time_fore <- append(line_time,line_time) 
MONASH <- rep("Model with Current Monash Setpoints", 720)
optmodel <- rep("Optimised Model", 720)
model_ab <- append(MONASH,optmodel) 

sep_2 <- data.frame("forecast" = forecast, 
           "time" = time_fore,
           'Model' = model_ab)

sep_2_final <- rbind(sep_2[1:720,],sep_2[854:1440,])



write.csv(sep_2_final, file = "9_2.csv")
